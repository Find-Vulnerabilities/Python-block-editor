...
pythonGenerator.forBlock['time_sleep'] = function(block, generator) {
  ...
  // Use await for mocked time.sleep
...
window.turtleAPI = {
  ...
  setheading: function(a) { this.angle = a - 90; }, // math 0 is right (-90 because we default pointing UP)
  ...
  color: function(c) {
    ...
    if(canvas) {
      ...
      canvas.getContext('2d').beginPath(); // break path to apply new color instantly
  ...
  stamp: function() {
    ...
    ctx.rotate(this.angle * Math.PI / 180 + Math.PI/2); // Make turtle point to direction
    ctx.font = "20px Arial";
  ...
  goto: function(args) {
    ...
    this.x = cx + dx;
    this.y = cy - dy; // math coordinates (up is positive)
    const ctx = canvas.getContext('2d');
    if(this.penDown) { ctx.lineTo(this.x, this.y); ctx.stroke(); }
    else { ctx.moveTo(this.x, this.y); }
    this._addToFill(this.x, this.y);
  ...
  circle: function(r) {
    ...
    if(!canvas) return;
    const ctx = canvas.getContext('2d');
    // Simplified circle for educational purpose
    ctx.beginPath();
  ...
  clear: function() {
    ...
    const ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    // standard turtle.clear() does not move the turtle
  },
  pos: function() {
    const canvas = document.getElementById('turtle-canvas');
...
function initPyodideWorker() {
  ...
  // The inline Web Worker for Pyodide
  const workerScript = `
...
sys.modules["turtle"] = turtle_mod
          ...
          postMessage({ type: 'stderr', text: str(err) });
        }
  ...
  pyodideWorker.onmessage = function(e) {
    ...
    if (data.type === 'ready') {
      ...
      consoleOutput.scrollTop = consoleOutput.scrollHeight;
    } else if (data.type === 'turtle' && window.turtleAPI) {
      let res = null;
      if(window.turtleAPI[data.cmd]) {
        res = window.turtleAPI[data.cmd](data.arg);
      }
...
document.getElementById('btn-stop').addEventListener('click', () => {
  ...
  initPyodideWorker(); // kill old worker and start a fresh one cleanly immediately!
});
...
pythonGenerator.forBlock['turtle_clear'] = function(block) { return "await turtle.clear()\n"; };
...
pythonGenerator.forBlock['turtle_xcor'] = function(block) { return ["await turtle.xcor()", pythonGenerator.ORDER_FUNCTION_CALL]; };
...
pythonGenerator.forBlock['turtle_ycor'] = function(block) { return ["await turtle.ycor()", pythonGenerator.ORDER_FUNCTION_CALL]; };
...
pythonGenerator.forBlock['turtle_penup'] = function(block) { return "await turtle.penup()\n"; };
...
pythonGenerator.forBlock['turtle_pendown'] = function(block) { return "await turtle.pendown()\n"; };
...
pythonGenerator.forBlock['turtle_begin_fill'] = function(block) { return "await turtle.begin_fill()\n"; };
...
pythonGenerator.forBlock['turtle_end_fill'] = function(block) { return "await turtle.end_fill()\n"; };
...
pythonGenerator.forBlock['turtle_heading'] = function(block) { return ["await turtle.heading()", pythonGenerator.ORDER_FUNCTION_CALL]; };
...
pythonGenerator.forBlock['turtle_stamp'] = function(block) { return "await turtle.stamp()\n"; };
